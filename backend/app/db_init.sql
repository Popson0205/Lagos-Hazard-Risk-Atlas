...
-- Run once against a PostGIS-enabled database (Supabase or local docker
-- postgis service). Supabase ships PostGIS already enabled, so the CREATE
-- EXTENSION below is a no-op there, but it's kept for local/other-provider
-- Postgres.
--
-- This file must match app/models.py exactly (table/column names) and seeds
-- the same data as app/seed.py (python -m app.seed) / data/catalogue/
-- hazard_layers.json — use this file when you can't run seed.py directly
-- (e.g. no shell access on Render's free tier). Re-running this script is
-- safe: every INSERT uses ON CONFLICT DO NOTHING.
--
-- Loaded via scripts/load_schema.sh, or paste this whole file into
...
-- Generic vector store for polygon/point hazard outputs — one row per
-- feature, keyed by layer_id (see app/models.py's HazardFeature docstring).
...
-- STAC-recipe expressions use rio-tiler's positional b1/b2/... band
-- convention (matching the order `assets` is given in), not literal asset
-- names — rio-tiler's MultiBaseReader always renames merged asset bands
-- positionally, and its expression parser treats any token matching
-- b<digits> (case-insensitively) as a band index, which Sentinel-2's real
-- asset names ("B03", "B08", ...) collide with. See
-- app/services/stac_client.stac_statistics_url's docstring and
-- app/services/hazard_recipes.py for the full explanation. `nodata: 0` is
-- likewise required so cloud-gap/scene-edge fill pixels (raw 0) aren't
-- counted as real data by the band-math expression.
